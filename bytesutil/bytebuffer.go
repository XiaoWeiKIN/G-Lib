...
import (
	...
	"github.com/XiaoWeiKIN/G-Lib/slicesutil"
)
...
// ReadCloser is a standard interface for stream readers.
//
// It mirrors filestream.ReadCloser from VictoriaMetrics, so the reader returned
// by ByteBuffer.NewReader can be passed to code expecting a file stream reader.
type ReadCloser interface {
	// Path returns an unique id for the reader source.
	Path() string

	// Read reads up to len(p) bytes into p.
	Read(p []byte) (int, error)

	// MustClose closes the reader.
	MustClose()
}
